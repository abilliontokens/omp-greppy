...
import { childEnv, resolveGreppyBinary, runGreppy } from "./greppy.js";
...
function list(params, key) {
    ...
    return raw.filter((item) => typeof item === "string").map((item) => item.trim()).filter((item) => item.length > 0);
}
/** Flags shared by the listing commands; order is irrelevant to greppy. */
function commonFlags(params, flags) {
    const args = [];
    ...
    if (flags.code === true && params["code"] === true)
        args.push("--code");
...
export const TOOL_SPECS = [
    ...
    },
    {
        ...
        build: (p) => {
            ...
            if (p["smart"] === true && depth !== undefined)
...
export function formatResult(spec, result, timeoutMs) {
    const stdout = result.stdout.trim();
    const stderr = result.stderr.trim();
    if (result.error !== undefined)
        return `${spec.name} failed: could not run greppy (${result.error})`;
    if (result.aborted)
        return `${spec.name} cancelled`;
    if (result.timedOut) {
        return `${spec.name} timed out after ${Math.round(timeoutMs / 1000)}s; the index may still be building — check /greppy-status.`;
    }
    if (result.code === 0)
        return clip(stderr.length > 0 && stdout.length > 0 ? `${stdout}\n\n${stderr}` : stdout || stderr || "(no output)");
    if (result.code === 1)
        return clip(stdout || stderr || "no match");
    if (result.code === EXIT_INDEX_NOT_READY) {
        ...
        if (spec.notReadyHint !== undefined)
            lines.push(spec.notReadyHint);
...
export function registerGreppyTools(pi, deps = {}) {
        ...
        pi.registerTool({
            ...
            execute: async (_toolCallId, params, signal, _onUpdate, ctx) => {
                ...
                if (typeof args === "string")
                    return text(`${spec.name} failed: ${args}`);
                const where = resolveCwd(p, ctx);
                if ("error" in where)
                    return text(`${spec.name} failed: ${where.error}`);
                const cwd = where.cwd;
