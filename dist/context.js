...
export function buildGreppyNote(indexed) {
...
export function appendNoteToMessages(messages, note) {
    for (let index = messages.length - 1; index >= 0; index -= 1) {
        ...
        if (Array.isArray(message.content)) {
            if (!message.content.some((part) => part.text === note))
...
export class RepoIndexCache {
    ...
    get(bin, root, now = Date.now()) {
        const entry = this.#entries.get(root);
        if (entry === undefined || (!entry.pending && now - entry.checkedAt > STATUS_TTL_MS))
            void this.refresh(bin, root);
...
export function noteForCwd(cwd, cache, resolveBinary = resolveGreppyBinary) {
